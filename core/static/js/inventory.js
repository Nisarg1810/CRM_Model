...
async function addTaskAssignmentRow() {
    const marketingSelect = document.getElementById('sellMarketingEmployee');
    const selectedEmployee = marketingSelect.options[marketingSelect.selectedIndex];
    
    if (!selectedEmployee || !selectedEmployee.value) {
        alert('Please select a marketing employee first');
        return;
    }
    
    // Load tasks for the selected employee if not already loaded
    if (selectedEmployee.value && (!window.availableTasks || window.availableTasks.length === 0)) {
        try {
            await loadEmployeeTasks(selectedEmployee.value);
        } catch (error) {
            console.error('Error loading tasks:', error);
            showNotification('Failed to load tasks for selected employee', 'error');
            return;
        }
    }
    ...
    taskRow.className = 'task-assignment-row mb-3 p-3 border rounded';
    taskRow.setAttribute('data-task-index', taskAssignmentCounter);
    
    // Get available tasks from the global variable (updated by loadEmployeeTasks)
    const tasksDropdownOptions = window.availableTasks || [];
    let taskOptions = '<option value="">Select a task...</option>';
    
    if (tasksDropdownOptions.length === 0) {
        taskOptions += '<option value="" disabled>No tasks available for this employee</option>';
    } else {
        tasksDropdownOptions.forEach(task => {
            taskOptions += `<option value="${task.name}" data-completion-days="${task.completion_days}">${task.name}</option>`;
        });
    }
    ...
    taskRow.innerHTML = `
        <div class="row g-2">
            <div class="col-md-4">
                ...
                <select class="form-control task-name" name="task_name_${taskAssignmentCounter}" 
                        onchange="updateCompletionDaysFromTask(this, ${taskAssignmentCounter})" required>
                    ${taskOptions}
                </select>
            </div>
            <div class="col-md-3">
                <label class="form-label">Completion Days <span class="text-danger">*</span></label>
                <input type="number" class="form-control completion-days" name="completion_days_${taskAssignmentCounter}" 
                       placeholder="Days" value="7" readonly required>
            </div>
            <div class="col-md-4">
                <label class="form-label">Assigned Employee</label>
                <input type="text" class="form-control assigned-employee" value="${selectedEmployee.text}" readonly>
                <input type="hidden" name="assigned_employee_${taskAssignmentCounter}" value="${selectedEmployee.value}">
            </div>
        </div>
    `;
...
// Function to update completion days when task is selected
function updateCompletionDaysFromTask(selectElement, taskIndex) {
    const selectedOption = selectElement.options[selectElement.selectedIndex];
    const completionDays = selectedOption.getAttribute('data-completion-days');
    
    if (completionDays) {
        const completionDaysInput = document.querySelector(`input[name="completion_days_${taskIndex}"]`);
...
function removeTaskAssignmentRow(index) {
...
function updateTaskAssignmentSummary() {
    const container = document.getElementById('taskAssignmentsContainer');
    const summary = document.getElementById('taskAssignmentSummary');
    const totalTasksSpan = document.getElementById('totalTasks');
    const taskStatus = document.getElementById('taskStatus');
    
    const taskRows = container.querySelectorAll('.task-assignment-row');
    const totalTasks = taskRows.length;
    
    if (totalTasks > 0) {
        summary.style.display = 'block';
        totalTasksSpan.textContent = totalTasks;
        taskStatus.textContent = totalTasks === 1 ? '1 Task Added' : `${totalTasks} Tasks Added`;
        taskStatus.className = 'badge bg-light text-dark';
    } else {
...
function updateTaskAssignmentInfo() {
    // This function is now used to update existing task assignment rows when marketing employee changes
    const marketingSelect = document.getElementById('sellMarketingEmployee');
    const selectedEmployee = marketingSelect.options[marketingSelect.selectedIndex];
    
    // Update all existing task assignment rows with new employee
    const taskRows = document.querySelectorAll('.task-assignment-row');
    taskRows.forEach(row => {
        const employeeInput = row.querySelector('input[readonly]');
        const hiddenInput = row.querySelector('input[type="hidden"][name*="assigned_employee"]');
        
        if (employeeInput && hiddenInput) {
            if (selectedEmployee && selectedEmployee.value) {
                employeeInput.value = selectedEmployee.text;
                hiddenInput.value = selectedEmployee.value;
            } else {
                employeeInput.value = '';
                hiddenInput.value = '';
            }
...
// Load tasks assigned to specific marketing employee
function loadEmployeeTasks(employeeId) {
    if (!employeeId) {
        window.availableTasks = window.allMarketingTasks || [];
        updateExistingTaskDropdowns();
        return Promise.resolve();
    }
    
    // Show loading state (button removed, but keeping functionality for auto-population)
    
    return fetch(`/api/employee/${employeeId}/tasks/`, {
        method: 'GET',
        headers: {
    ...
    .then(data => {
        ...
        if (data.success) {
            window.availableTasks = data.tasks;
            updateExistingTaskDropdowns();
            
            // Auto-populate all marketing tasks when employee is selected
            autoPopulateAllTasks(data.tasks, data.employee_name);
            
            // Show notification about loaded tasks
            if (data.tasks.length > 0) {
                showNotification(`Auto-added ${data.tasks.length} marketing tasks for ${data.employee_name}`, 'success');
            } else {
                showNotification(`No marketing tasks available`, 'info');
            }
        } else {
            console.error('API returned error:', data.error);
            showNotification(`Failed to load tasks: ${data.error || 'Unknown error'}`, 'error');
            window.availableTasks = window.allMarketingTasks || [];
            updateExistingTaskDropdowns();
        }
    })
    ...
    .finally(() => {
        // Add Task button removed - no need to restore
    });
}

// Auto-populate all marketing tasks when employee is selected
function autoPopulateAllTasks(tasks, employeeName) {
    // Clear existing task assignment rows
    const container = document.getElementById('taskAssignmentsContainer');
    container.innerHTML = '';
    taskAssignmentCounter = 0;
    
    // Add a task assignment row for each marketing task
    tasks.forEach(task => {
        taskAssignmentCounter++;
        ...
        taskRow.innerHTML = `
            <div class="row g-2">
                <div class="col-md-4">
                    <label class="form-label">Task Name <span class="text-danger">*</span></label>
                    <input type="text" class="form-control task-name" name="task_name_${taskAssignmentCounter}" 
                           value="${task.name}" readonly required>
                </div>
                <div class="col-md-3">
                    <label class="form-label">Completion Days <span class="text-danger">*</span></label>
                    <input type="number" class="form-control completion-days" name="completion_days_${taskAssignmentCounter}" 
                           value="${task.completion_days}" readonly required>
                </div>
                <div class="col-md-4">
                    <label class="form-label">Assigned Employee <span class="text-danger">*</span></label>
                    <input type="text" class="form-control assigned-employee" name="assigned_employee_${taskAssignmentCounter}" 
                           value="${employeeName}" readonly required>
                </div>
            </div>
        `;
...
function updateTaskCounter() {
    ...
    const taskCounterElement = document.querySelector('.badge');
    if (taskCounterElement) {
        taskCounterElement.textContent = `${totalTasks} Task${totalTasks !== 1 ? 's' : ''} Added`;
    }
    
    // Update the total tasks display
    const totalTasksElement = document.querySelector('[class*="Total Tasks"]');
    if (totalTasksElement) {
        totalTasksElement.textContent = `Total Tasks: ${totalTasks}`;
    }
}

// Update existing task dropdowns with new task list
function updateExistingTaskDropdowns() {
    const taskDropdowns = document.querySelectorAll('.task-assignment-row .task-name');
    ...
    taskDropdowns.forEach(dropdown => {
        ...
        window.availableTasks.forEach(task => {
            const option = document.createElement('option');
            option.value = task.name;
            option.setAttribute('data-completion-days', task.completion_days);
            option.textContent = task.name;
            
            // Restore selection if it exists in new task list
            if (task.name === currentValue) {
...
function populateSellLandForm(land, clients, employees) {
    ...
    // Store all marketing tasks as backup
    if (window.availableTasks && !window.allMarketingTasks) {
        window.allMarketingTasks = [...window.availableTasks];
    }
    
    // Add event listener for marketing employee selection to load their tasks
    employeeSelect.addEventListener('change', function() {
        const employeeId = this.value;
        if (employeeId) {
            loadEmployeeTasks(employeeId).then(() => {
                updateTaskAssignmentInfo(); // Update existing task assignments
            });
        } else {
            window.availableTasks = window.allMarketingTasks || [];
            updateExistingTaskDropdowns();
...
function populateSellLandFormWithExistingSale(land, clients, employees, sale, installments) {
    ...
    // Load existing task assignments for this land
    if (typeof loadExistingTaskAssignments === 'function') {
        loadExistingTaskAssignments(land.id);
    }
...
function enableSellFormEdit() {
    ...
    // Update total percentage to include paid installments
                ...
                <div class="cheque-photo mb-4 p-3 bg-light rounded">
                    ...
                    <div class="text-center">
                        <img src="${paymentDetails.cheque_photo}" 
                             ...
                             onclick="downloadChequePhoto('${paymentDetails.cheque_photo}')"
                             title="Click to download photo">
                        <div class="mt-3">
                            <button class="btn btn-sm btn-primary" onclick="downloadChequePhoto('${paymentDetails.cheque_photo}')">
                                <i class="bi bi-download me-1"></i>Download Photo
                            </button>
                            <button class="btn btn-sm btn-outline-secondary ms-2" onclick="openImageModal('${paymentDetails.cheque_photo}')">
...
function downloadChequePhoto(imageSrc) {
    try {
        // Create a temporary anchor element for download
        const link = document.createElement('a');
        link.href = imageSrc;
        
        // Extract filename from the URL or create a default name
        const urlParts = imageSrc.split('/');
        const filename = urlParts[urlParts.length - 1] || 'cheque_photo.jpg';
        
        // Set download attribute with a meaningful filename
        link.download = `cheque_photo_${new Date().toISOString().split('T')[0]}_${filename}`;
        ...
        showNotification('Cheque photo download started!', 'success');
        
    } catch (error) {
        console.error('Error downloading cheque photo:', error);
        showNotification('Error downloading photo. Please try again.', 'error');
    }
}
...
function openImageModal(imageSrc) {
            ...
            <div class="modal-dialog modal-xl modal-dialog-centered">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-primary" onclick="downloadChequePhoto(document.getElementById('fullSizeImage').src)">
    ...
    // Force z-index to appear above payment details modal
    setTimeout(() => {
        const modalElement = document.getElementById('imageModal');
        if (modalElement) {
            modalElement.style.zIndex = '1080'; // Higher than payment details modal (1070)
        }
        const backdrop = document.querySelector('.modal-backdrop:last-child');
        if (backdrop) {
                            ...
                            <div class="info-section mb-4">
                                ...
                                <div class="info-grid">
                                    <div class="info-item">
                                        ...
                                        <span class="info-value">${land.soda_tarikh || 'Not set'}</span>
                                    </div>
                                    <div class="info-item">
                                        <span class="info-label">Banakhat Date</span>
                                        <span class="info-value">${land.banakhat_tarikh || 'Not set'}</span>
                                    </div>
                                    <div class="info-item">
                                        <span class="info-label">Dastaveg Date</span>
                                        <span class="info-value">${land.dastavej_tarikh || 'Not set'}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
...
async function showInstallmentsModal(landId) {
    ...
    try {
        // Show loading state with proper z-index handling
        ...
        // Force z-index after modal is shown
        setTimeout(() => {
            const modalElement = document.getElementById('manageInstallmentsModal');
            if (modalElement) {
...
function updateTotalPercentage() {
    ...
    rows.forEach(row => {
        // Check for editable percentage inputs (new installments)
        const percentageInput = row.querySelector('input[id^="percentage"]');
        ...
        // Check for read-only percentage inputs (paid installments)
        const readOnlyPercentageInput = row.querySelector('input.form-control-readonly[type="number"]');
        if (readOnlyPercentageInput && readOnlyPercentageInput.value) {
            const percentage = parseFloat(readOnlyPercentageInput.value) || 0;
            totalPercentage += percentage;
        }
    });
...
function getInstallmentData() {
    ...
    rows.forEach((row, index) => {
        // Check if this is a paid installment (read-only)
        const isPaidInstallment = row.classList.contains('paid-installment');
        
        if (isPaidInstallment) {
            // For paid installments, get data from read-only inputs
            const paymentMethodInput = row.querySelector('input.form-control-readonly[value*="Cash"], input.form-control-readonly[value*="Cheque"], input.form-control-readonly[value*="Bank Transfer"], input.form-control-readonly[value*="UPI"], input.form-control-readonly[value*="Other"]');
            const percentageInput = row.querySelector('input.form-control-readonly[type="number"]');
            const dueDateInput = row.querySelector('input.form-control-readonly[type="date"]');
            
            if (paymentMethodInput && percentageInput && dueDateInput) {
                const paymentMethod = getPaymentMethodValue(paymentMethodInput.value);
                const percentage = parseFloat(percentageInput.value) || 0;
            ...
            // For editable installments, get data from form inputs
            ...
            if (daysInput && paymentMethodSelect && percentageInput && dueDateInput) {
                ...
                if (percentage > 0) {
                    installments.push({
                        ...
                        percentage: percentage,
                        due_date: dueDate,
                        status: 'pending' // Mark as pending for new/editable installments
                    });
                }
            }
        }
...
// Get task assignment data from the form
function getTaskAssignmentData() {
    const taskRows = document.querySelectorAll('#taskAssignmentsContainer .task-assignment-row');
    const taskAssignments = [];
    
    console.log('Getting task assignment data...');
    console.log('Found task rows:', taskRows.length);
    
    taskRows.forEach((row, index) => {
        const taskNameElement = row.querySelector('.task-name');
        const completionDaysElement = row.querySelector('.completion-days');
        const assignedEmployeeElement = row.querySelector('input[type="hidden"][name*="assigned_employee"]');
        
        console.log(`Row ${index + 1}:`, {
            taskNameElement: taskNameElement,
            completionDaysElement: completionDaysElement,
            assignedEmployeeElement: assignedEmployeeElement
        });
        
        if (taskNameElement && completionDaysElement && assignedEmployeeElement) {
            const taskName = taskNameElement.value.trim();
            const completionDays = parseInt(completionDaysElement.value) || 0;
            const assignedEmployeeId = assignedEmployeeElement.value;
            ...
            if (taskName && completionDays > 0 && assignedEmployeeId) {
...
function convertInstallmentsToEditable() {
    ...
    // Convert each read-only installment to editable (except paid ones)
    readOnlyRows.forEach((row, index) => {
        ...
        // Check if installment is paid - if so, keep it read-only
        const isPaid = status === 'Paid';
        
        if (isPaid) {
            // Keep paid installments as read-only with special styling
            const rowHtml = `
                <div class="installment-row read-only-installment paid-installment" id="installmentRow${installmentRowCount}">
                    <div class="installment-fields">
                        <div class="installment-field">
                            <label>Payment Method</label>
                            <input type="text" class="form-control form-control-readonly" value="${paymentMethod}" readonly>
                        </div>
                        <div class="installment-field">
                            <label>%</label>
                            <input type="number" class="form-control form-control-readonly" value="${percentage}" readonly>
                        </div>
                        <div class="installment-field">
                            <label>Due Date</label>
                            <input type="date" class="form-control form-control-readonly" value="${dueDate}" readonly>
                        </div>
                        <div class="installment-field">
                            <label>Status</label>
                        ...
                        <div class="installment-field">
                            <span class="text-muted small">
                                <i class="bi bi-lock me-1"></i>Paid - Cannot Edit
                            </span>
                        </div>
                    </div>
                </div>
            ...
            // Convert unpaid installments to editable
...
window.downloadChequePhoto = downloadChequePhoto;
